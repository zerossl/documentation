@extends('\_layouts.master') @section('body\_class', 'documentation help') @section('title', 'API Method: Download Certificate Inline - ZeroSSL') @section('canonical\_url', 'https://zerossl.com/documentation/api/download-certificate-inline') @section('meta\_description', 'Learn how to download certificate files as inline JSON objects using the ZeroSSL API.') @section('body')

@include('\_partials.documentation-sidebar', \['active\_article' => $active\_article\])

# [REST API](/documentation/api "ZeroSSL Documentation") Download Certificate (inline)
...
{
"certificate.crt": "---BEGIN CERTIFICATE---{primary\_certificate}---END CERTIFICATE---",
"ca\_bundle.crt": "---BEGIN CERTIFICATE---{certificate\_bundle}---END CERTIFICATE---"
}
